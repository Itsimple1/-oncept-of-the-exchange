...
async def back_change(message: Message, state: FSMContext):
    await state.clear()
...
async def check_is_tel(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "\nВ случае неверно введенного номера при покупке тех-👨‍💼Поддержка👨‍💼 не сможет вам помочь!",
...
async def confirm_change(message: Message, state: FSMContext):
    ...
    kb = [[types.KeyboardButton(text="👉К продаже👈"), types.KeyboardButton(text="📄О проекте📄"),
           types.KeyboardButton(text="👨‍💼Поддержка👨‍💼"), types.KeyboardButton(text="👨‍💻Аккаунт👨‍💻")], ]
    keyboard = types.ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)
    await api_db.sql_bd_sellers.seller_add_tel(message.from_user.id, data['input_tel'])
...
async def mistake_in_tel(message: Message, state: FSMContext):
